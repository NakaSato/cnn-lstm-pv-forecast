I'll create a function to test the data preprocessing pipeline and model predictions. I'll provide a test script that can verify if your PV forecasting workflow is working correctly.

## Step-by-step solution:

1. Create a new test script file called `test_pipeline.py` that will:

   - Test the data preprocessing functions
   - Test the model loading and prediction
   - Validate the outputs at each step with detailed diagnostics
   - Generate visual reports

2. Implement functions to test preprocessing, model loading, and inference

3. Add command-line arguments to run specific test components

4. Include utility functions to visualize results

### [test_pipeline.py](file:///Users/chanthawat/Development/py-dev/cnn-lstm-pv-forecast/test_pipeline.py)

This file will contain the comprehensive test framework:
...
from data_preprocessing import (load_data, remove_outliers, interpolate_missing_values,
                               normalize_data, create_sequences, preprocess_data)
from weather_utils import load_weather_data_v2, align_pv_weather, extract_advanced_features
...
Here's how to use the test script:

1. To run the complete end-to-end test:

```
python test_pipeline.py --data data/inverter/INVERTER_01.csv --model models/hybrid_model --weather data/weather/weather_data.csv
```

2. To test only the data loading:

```
python test_pipeline.py --test-type data-loading --data data/inverter/INVERTER_01.csv
```

3. To test only the preprocessing:

```
python test_pipeline.py --test-type preprocessing --data data/inverter/INVERTER_01.csv --weather data/weather/weather_data.csv
```

4. To test only model loading:

```
python test_pipeline.py --test-type model-loading --model models/hybrid_model
```

5. To test only prediction:

```
python test_pipeline.py --test-type prediction --data data/inverter/INVERTER_01.csv --model models/hybrid_model
```

If you don't provide file paths, the script will try to find appropriate files in the standard locations.

The test script will generate diagnostic information and visualizations in a `test_results` directory. These can help identify any issues with your data, preprocessing, or model.
